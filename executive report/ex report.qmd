...
To make comparisons across models and evaluation settings clearer, Table 1 summarizes the key results discussed in Sections 6.1–6.4. The goal is not to identify a single best model, but to highlight trade-offs between accuracy, macro-level performance, sensitivity to N1, robustness under subject-level evaluation, and compute cost.

This summary highlights the central theme of the results: performance depends strongly on evaluation setting, sequence modeling, and compute constraints, rather than architecture alone.
...
results_tbl <- tribble(
  ~Section, ~Model, ~Setting, ~Accuracy, ~Macro_F1, ~N1_Recall, ~Note,
  
  "6.1", "XGBoost", "Subject-level split", 0.9112, 0.6785, "0.160",
  "Strong baseline; weakest on N1",
  
  "6.2", "Attention", "Fixed-split", 0.8714, 0.7174, "0.750",
  "Higher N1 recall than U-Sleep-inspired",
  
  "6.2", "U-Sleep-inspired", "Fixed-split", 0.8902, 0.7261, "0.660",
  "Best overall fixed-split deep model",
  
  "6.3", "Attention", "Reduced-compute subject-level CV + HMM (w=0.0)", 0.8461, 0.5994, "48/170",
  "Baseline reduced-CV setting",
  
  "6.3", "Attention", "Reduced-compute subject-level CV + HMM (w=0.5)", 0.8600, 0.6069, "27/170",
  "Best macro F1 under CV",
  
  "6.3", "Attention", "Reduced-compute subject-level CV + HMM (w=1.0)", 0.8646, 0.6064, "26/170",
  "Best accuracy under CV",
  
  "6.3", "Attention", "Reduced-compute subject-level CV + HMM (w=2.0)", 0.8541, 0.5828, "22/170",
  "Over-smoothed; lower macro F1",
  
  "6.4", "U-Sleep-inspired", "Reduced-compute subject-level CV + HMM (w=0.0)", 0.2836, 0.1606, "0.790",
  "Exploratory only; unstable under reduced compute",
  
  "6.4", "U-Sleep-inspired", "Reduced-compute subject-level CV + HMM (w=0.5)", 0.4570, 0.1922, "0.000",
  "Exploratory only; N1 collapsed after HMM"
) %>%
  mutate(
    Accuracy = sprintf("%.4f", Accuracy),
    Macro_F1 = sprintf("%.4f", Macro_F1)
  )

kable(
  results_tbl,
  col.names = c("Section", "Model", "Setting", "Accuracy", "Macro F1", "N1 Recall", "Notes"),
  align = c("c", "l", "l", "c", "c", "c", "l"),
  caption = "Summary of Section 6 results across XGBoost baseline, fixed-split deep models, and reduced-compute subject-level cross-validation with HMM decoding."
)
```
...
We evaluate the Attention model and the U-Sleep-inspired model under a fixed subject-level split. Both models demonstrate strong multi-class learning behavior, and the U-Sleep-inspired model achieves the highest overall performance (accuracy 0.8902, macro F1 0.7261), while the Attention model remains competitive (accuracy 0.8714, macro F1 0.7174).

However, the comparison changes when focusing on N1. The Attention model achieves higher N1 recall, indicating greater sensitivity to the most difficult and underrepresented stage, while U-Sleep provides more balanced performance across classes. This highlights that aggregate metrics do not fully capture clinically relevant trade-offs.

## 6.3 Reduced-Compute Subject-Level Cross-Validation and HMM Results for Attention

We evaluate the Attention model under reduced-compute subject-level cross-validation, which provides a more realistic estimate of generalization than a single split.

Under this setting, performance drops to accuracy 0.8461 and macro F1 0.5994, confirming that fixed-split results are optimistic in small-subject datasets.

Adding HMM decoding improves performance under moderate transition weights. Transition weight 0.5 achieves the highest macro F1 (0.6069), while 1.0 achieves the highest accuracy (0.8646). However, stronger transition priors reduce N1 recall from 0.282 to as low as 0.129. This shows a clear trade-off: sequence-aware decoding improves global consistency but suppresses short, difficult stages.

## 6.4 Reduced-Compute Subject-Level Cross-Validation Results for U-Sleep

These runs are exploratory only because the heavier model was trained under capped train and evaluation budgets.

Under reduced-compute subject-level cross-validation, the U-Sleep-inspired model shows unstable and significantly degraded performance compared with the fixed-split setting. Accuracy ranges from 0.2836 to 0.4570 and macro F1 from 0.1606 to 0.1922. While HMM decoding slightly improves aggregate metrics, it leads to severe degradation in N1 recall, including cases where N1 predictions disappear entirely.

These results should not be interpreted as a definitive comparison with the Attention model, but rather as evidence that the heavier architecture requires a less constrained compute budget for reliable evaluation.

## 6.5 Trade-Off Analysis

Across all settings, no single model dominates. XGBoost provides a strong and interpretable baseline but struggles on N1. The U-Sleep-inspired model achieves the best fixed-split performance but requires more compute. The Attention model, particularly with HMM decoding, provides the clearest insight into subject-level generalization and sequence modeling.

Overall, the results emphasize that conclusions depend critically on evaluation design, sequence modeling assumptions, and compute budget.

# 7. Future Steps

Future work focuses on extending the current findings rather than re-motivating the same design choices. The most important next step is to evaluate the U-Sleep-inspired model under reduced-compute subject-level cross-validation with a sufficient training budget, as current results are inconclusive.
...
For this project, “good enough for further development” is defined as evidence of meaningful multi-class performance under subject-level evaluation together with stable behavior across key sleep stages. Under this definition, the results suggest that EEG-only sleep staging is sufficiently promising to justify continued internal development, but not yet robust enough for deployment without further refinement.
...
![Class distribution of sleep stages (Subject SC400).](../data%20visualization/sub0_class_distribution.png)

![Sample hypnogram showing sleep stage transitions over time.](../data%20visualization/hypnogram.png)

![Representative 30-second EEG segments for different sleep stages.](../data%20visualization/sample_waveform.png)

![Power Spectral Density (PSD) analysis across sleep stages.](../data%20visualization/sub0_psd_by_stage.png)
