...
def load_sleep_edf_subject(psg_file, hyp_file):
    ...
    # Create epochs
    tmax = 30. - 1. / raw.info['sfreq']  # 30s epochs
    epochs = mne.Epochs(raw=raw, events=events, event_id=valid_event_id, 
                        tmin=0., tmax=tmax, baseline=None, preload=True, verbose=False)
    
    # Map raw labels to our 0-4 indices
    # We need to map the internal integer IDs (from event_id) to our STAGE_MAPPING values
